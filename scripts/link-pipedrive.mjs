#!/usr/bin/env node
// link-pipedrive.mjs — incoming internal links for /startup/pipedrive-alternatives
...
const TARGET = "/startup/pipedrive-alternatives";
const A = (txt) => `<a href="${TARGET}">${txt}</a>`;
const plan = {
  "hubspot-alternatives": {
    frag: "which suits founder-led outbound",
    insert: ` If Pipedrive is on your shortlist too, we priced ${A("its current tiers and add-ons")} the same way.`,
  },
  "choosing-the-right-crm-for-your-startup": {
    frag: "automate lead generation, and enhance customer satisfaction",
    insert: ` If a dedicated sales pipeline is the direction, start with our breakdown of ${A("Pipedrive and its rivals")}.`,
  },
};
...
for (const [slug, ins] of Object.entries(plan)) {
  ...
  console.log(`  ✓ ${slug} → pipedrive-alternatives`);
  applied++;
  if (!DRY) {
